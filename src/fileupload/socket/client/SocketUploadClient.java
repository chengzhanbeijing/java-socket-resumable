...
public class SocketUploadClient{
	...
	private boolean aa(Part filePart){
		boolean flag = true;
		try {
	...
	protected void doGet1(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, InterruptedException {
		String response1 = "";
	    //final Part filePart = request.getPart("file");
	    Collection<Part> coll = request.getParts();
    	//final String fileName = getFileName(filePart);
	    boolean flag1 = true;
	    List<Part> arrList = new ArrayList<Part>();
	    for(Part filePart1:coll){
	    	arrList.add(filePart1);
	    }
	    countDownLatch = new CountDownLatch(arrList.size());
	    SocketUtils.countDownLatch = countDownLatch;
	    for(int i=0;i<arrList.size();i++){
	    	SocketUtils cli = new SocketUtils();
	    	cli.setFilePart(arrList.get(i));
	    	Thread t = new Thread(cli,"thread"+UUID.randomUUID());
	    	t.start();
	    	//cli.aa(cli.getFilePart());
	    }
	    	//t.setDaemon(false);
	    	/*if(!cli.flag){
	    		flag1 = false;
	    ...
	    response.getWriter().print(flag1?"upload successful":"upload failed"); 
	}
